...
dir <- '/Volumes/Seagate_CWG/cgao13_KS2_182838-182839/RNAseq/quants'
...
plot + geom_label_repel(aes(label=gsub("_.*", "", dds$names)), show.legend = FALSE)

# collapse technical replicates
dds$sample <- factor(gsub("_.*", "", dds$names))
...
#dds.coll.filtered$condition <- relevel(dds.coll.filtered$condition, ref="Mut")
plot <- plotPCA(vst(dds.coll.filtered))
plot + geom_label_repel(aes(label=gsub("_.*", "", dds.coll.filtered$names)), show.legend = FALSE)
...
write.csv(diffexp.subset,'RNAseq_signif_CWG-quant_CWG-code.csv')
